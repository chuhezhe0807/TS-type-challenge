...
type sc = StringToUnion<'ABC'>

type StringToUnion<S extends string> = S extends `${infer First}${infer Rest}` ? First | StringToUnion<Rest> : S

type AllCombinations<S extends string, T extends string = StringToUnion<S>, U extends T = T> = S extends `${infer First}${infer Rest}`
  ? U extends U
